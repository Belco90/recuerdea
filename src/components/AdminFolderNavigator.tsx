...
import { Check, ChevronRight, Folder, Play } from 'lucide-react'
...
export function AdminFolderNavigator({
	...
	return (
		<Stack gap={5}>
			...
			{hasPicked && (
				<StickyFooter count={picked.size} onSave={() => onSave([...picked])} onCancel={onCancel} />
			)}
		</Stack>
	)
}
...
function FileGrid({
	...
	files: ReadonlyArray<AdminFileItem>
	picked: ReadonlySet<number>
	blocked: ReadonlySet<number>
	onToggle: (fileid: number) => void
}) {
...
function StickyFooter({
	...
	return (
		...
		>
			...
			<Button colorPalette="accent" size="sm" onClick={onSave}>
				Guardar ({count})
			</Button>
		</Box>
	)
