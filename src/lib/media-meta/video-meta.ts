...
const EMPTY: VideoMeta = { width: null, height: null, location: null }
...
function parseMoov(view: DataView, start: number, end: number): VideoMeta {
	...
	return { width: dims.width, height: dims.height, location }
}
...
function scanForMoov(view: DataView): VideoMeta {
	...
	for (let i = 4; i + 4 <= view.byteLength; i++) {
		...
		if (result.width !== null || result.location !== null) return result
	}
	return EMPTY
}
